...
def train(model, data, output_path, args, loss_func, score_func):
    ...
    # train
    print('before train:')
    best_dev_score = evaluate(model, data, score_func, args.batch_size, on='dev')
    best_epoch = -1
    print()
    ...
    for epoch in range(args.n_epochs):
        ...
        if cond1 or cond2:
            dev_score = evaluate(model, data, score_func, args.batch_size, on='dev')
...
def evaluate(model, data, score_func, batch_size, on='dev'):
    ...
    model.eval()
    n_epochs = user.shape[0] // batch_size + 1
    with torch.no_grad():
        # pos_pred = model(pos_features)
        # neg_pred = model(neg_features)
        # score = score_func(pos_pred, neg_pred)
        epoch_score = torch.tensor(0.0, device=user.device)
        for i, (pos_x, neg_x) in enumerate(batcher(pos_features, neg_features, batch_size=batch_size, shuffle=False)):
            pos_pred = model(pos_x)
            neg_pred = model(neg_x)
            score = score_func(pos_pred, neg_pred)
            if i == n_epochs - 1:
                epoch_score += score * (user.shape[0] % batch_size)
            else:
                epoch_score += score * batch_size
        score = epoch_score / user.shape[0]
...
def parse_args():
    ...
    parser.add_argument('--dataset', default='CA_filtered',
...
if __name__ == "__main__":
    ...
    # create model
    model = DeepFM(n_features=n_features, n_fields=n_fields, h1=32, h2=16, embedding_dim=args.embedding_dim)
    ...
    # write results
    save_results('./' + args.dataset + '_deepfm.txt', args, best_dev_score, test_score, dev_hit5, dev_hit10, test_hit5, test_hit10)
